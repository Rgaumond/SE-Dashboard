...
const styleNote = (propName,expandType) =>{
    if(!expandType)
        positionFLoat(propName, "85vh","30vw", "47%","80px");
...
const positionFLoat = (propName,height,width, left, top) =>{
    $(`#noteContainer-${propName}`).css({
        "position": "absolute","height": height, "width": width, "left": left, "top": top });
        $(`#noteContainer-${propName}`).resizable({ minHeight: 150, minWidth: 150 }).draggable({ handle: '.ql-toolbar' });
