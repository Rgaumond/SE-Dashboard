const generateList = (propName) => {
    ...
    if (propName === "presentations") {
        if (presentations.length === 0) {
            ...
            $(`#card-${propName}`).append(generatePresentationList());
    }
    else if (customer[propName].length > 0) {
        let ct = "";
        $.each(customer[propName], (index, listItem) => {
            if (propName === "contacts")
                ct += contactListItem(listItem, index);
            else if (propName === "documents")
                ct += documentListItem(listItem, index);
        });
        $(`#card-${propName}`).append(ct);
    }
    else
        $(`#card-${propName}`).append(`<div id='card-${propName}-listEmpty'>No ${propName}</div>`);
};
...
const generatePresentationList = () => {
    ...
    if (presentations.length === 0) {
        ...
        if (presoCount > 0)
            $(`#card-presentations`).append(ct);
        else
            $(`#card-presentations`).append(`<div id='card-presentations-listEmpty'>No presentations</div>`);
    }
};
...
const generateDetailsList = (listName) => {
    ...
    let ct = "";
    let tarGetArr;
    if (listName === "briefDetails")
        tarGetArr = briefDetails;
    else if (listName === "abbrevBriefDetails")
        tarGetArr = abbrevBriefDetails;
    else
        tarGetArr = solutionDetails;
    $.each(tarGetArr, (i, link) => {
        let classN = "";
        customer[link.propName] = customer[link.propName] ?? initiateObject();
        if (link.propName !== "dependencies" && (customer[link.propName] === "" || customer[link.propName] === "<p><br></p>")) {
            classN = " required";
            if (link.propName !== "finalContent" && !requiredDiscoveryInfo.find(obj => obj === link.propName))
                requiredDiscoveryInfo.push(link.propName);
        }
        else
            requiredDiscoveryInfo.splice(requiredDiscoveryInfo.indexOf(link.propName), 1);
        ...
        if ($(`#noteContainer-${link.propName}`).length===0)
            buildNoteSection(link.propName, link.title, addParentListAttribute, [link.propName, listName]);
    });
