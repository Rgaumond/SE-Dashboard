    ...
    // ACTIVE CODING
       ...
       expandNote(propName);
        ...
        if (propType === "checkbox")
            ct = buildCheckbox(propName, eval(propName + "Options").options, "", "checkedChanged(this)", propName);
        else if (propType === "select")
            if (propName === "accountManager")
                ct = buildSelect(propName, ArrayUtilities.sortByTextValue(eval(propName + "Options").options, 'text'), "", "selectChanged(this)", propName);
            else
                ct = buildSelect(propName, eval(propName + "Options").options, "", "selectChanged(this)", propName);
        ...
        $(`.dialog`).css({ "left": $(`#card-${propName}`).offset().left + "px", "top": $(`#card-${propName}`).offset().top - 10 });
