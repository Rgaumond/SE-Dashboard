...
const buildCustomerFilterInputs = () => {
    ...
    if(localStorage.getItem("engineeringCustomerName"))
        name = localStorage.getItem("engineeringCustomerName");
    $(`#filterContainer`).append(`${buildInput('filter-cieName', name, 'Customer', "applyfilter('cieName')")}`);
    $(`#filterContainer`).append(`<div class='filteredOn-clear' onclick='clearCustomerFilter()'><i class='fa fa-filter' aria-hidden='true'></i></div>`);
};
