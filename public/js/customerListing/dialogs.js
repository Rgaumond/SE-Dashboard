...
const addCustomerValidate = () => {
  if ($("#input-cieName").val() === "") alert("You must enter a name");
    ...
    $.each(customers, (index, customer) => {
      if (newName === customer.name.toLowerCase()) itExist = true;
    });
    if (itExist) alert("this customer already exist");
    else {
      let accountM = ArrayUtilities.findObjByProp(
        accountManagerOptions.options,
        "value",
        parseInt($("#select-accountManagerSelect").val())
      );
      let IAM = ArrayUtilities.findObjByProp(
        iamTypeOptions.options,
        "value",
        parseInt($("#select-iamType").val())
      );
      dialog.disintegrate();
