const dialog = {
    load(title, content, buttonAction, buttonLabel, callback) {
        dialog.disintegrate();
        $("body").append(`<div class='dialog-container'><div class='dialog'><div class='dialog__title'>${title}<div class='dialog__title-close' onclick='dialog.disintegrate()'>
        ...
        if (buttonAction)
            dialog.button(buttonLabel, buttonAction);
        if (callback)
            callback();
    },
    shell() {
    ...
    show() {
        dialog.veil.show(); dialog.container.show(); $(".veilCloser").show();
    },
    button(label, cta) {
        let ct = "<button class='dialog--bt' id='dialog--bt'>" + label + "</button>";
